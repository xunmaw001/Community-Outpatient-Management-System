    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="科室类型" prop="keshileixing">
          <el-select :disabled="ro.keshileixing" v-model="ruleForm.keshileixing" placeholder="请选择科室类型">
            <el-option
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="医生编号" prop="yishengbianhao">
          <el-select :disabled="ro.yishengbianhao" @change="yishengbianhaoChange" v-model="ruleForm.yishengbianhao" placeholder="请选择医生编号">
            <el-option
                v-for="(item,index) in yishengbianhaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.yishengbianhao" label="医生编号" prop="yishengbianhao">
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.keshitupian" label="科室图片" prop="keshitupian">
          <file-upload
          tip="点击上传科室图片"
          action="file/upload"
          :limit="3"
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.keshitupian" label="科室图片" prop="keshitupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.keshitupian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/yisheng/yishengbianhao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.yishengbianhaoOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    // 下二随
    yishengbianhaoChange () {
      this.$http({
        url: `follow/yisheng/yishengbianhao?columnValue=`+ this.ruleForm.yishengbianhao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.keshijieshao = this.ruleForm.keshijieshao.replace(reg,'../../../phpbm0n4/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
